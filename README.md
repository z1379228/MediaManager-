...
目前來源版本為開發版 29.0（核心相容版本 29.0.0）。28.0 及更早版本維持在各自的
`Version/Development/<major>.<minor>` 目錄，不覆寫歷史產物；29.0 尚未建立 Stable
或 Testing 公開附件。

平台無關的桌面媒體管理器。平台功能將由受隔離、具最小權限的 MOD 提供；核心負責安全、工作、儲存、網路與憑證代理。

29.0 已完成 Development 打包前的離線品質驗證：Ruff、882 項測試、版本／MOD／網站／
依賴／污染稽核均通過；Development 成品維持 `SAFE_MODE`，不是 Stable 正式版。
