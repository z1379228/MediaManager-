...
目前來源版本為測試版 1.0（核心相容版本 11.0.0），核心與網站功能分離。開發與
測試成品維持 `SAFE_MODE`，目前沒有可宣告的 Stable 正式版。GitHub 的
[`test-v1.0.0`](https://github.com/z1379228/MediaManager-/releases/tag/test-v1.0.0)
是既有測試版 Pre-release；發布後加入的動畫瘋來源變更尚未重新封裝，
不能把舊附件當成已包含新功能。已公開附件一律不可覆寫，重建必須使用新版本號。
目前來源包含：
