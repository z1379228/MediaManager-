...
def _ani_gamer_offline_actions_item(
    context: object, owner: object | None
) -> SelfCheckItem:
    """Verify current AniGamer archive controls without creating the workspace."""

    try:
        feature_statuses = {
            status.provider_id: status for status in context.features.statuses()
        }
        parent = feature_statuses.get("ani-gamer")
        offline = feature_statuses.get("ani-gamer-offline")
    except (AttributeError, RuntimeError):
        parent = None
        offline = None
    if parent is None or offline is None:
        return _item(
            "ui.ani_gamer_offline_actions",
            "block",
            "動畫瘋番劇儲存 MOD 未載入",
            "動畫瘋父 MOD 或 ani-gamer-offline 子 MOD 缺少執行狀態。",
            "ui.ani_gamer_offline.restore",
        )
    if owner is None:
        return _item(
            "ui.ani_gamer_offline_actions",
            "warning",
            "未附加動畫瘋工作區",
            "核心 MOD 狀態可用；從主視窗執行可再核對保存與取消按鈕。",
            "ui.ani_gamer_offline.open",
        )
    panel = owner.optional_workspace_manager.panels.get("ani-gamer")
    if panel is None:
        valid = not parent.enabled
        return _item(
            "ui.ani_gamer_offline_actions",
            "pass" if valid else "block",
            "動畫瘋工作區按需關閉" if valid else "動畫瘋工作區未建立",
            (
                "父 MOD 停用時不建立工作區，也不啟動離線作業。"
                if valid
                else "父 MOD 已啟用，但動畫瘋工作區尚未同步建立。"
            ),
            "ui.optional_workspaces.sync" if not valid else "",
        )

    try:
        series_selected = panel.selected_result() is not None
        episode_selected = panel.selected_episode() is not None
        busy = bool(panel.busy)
        expected_ready = bool(
            parent.enabled
            and offline.enabled
            and series_selected
            and episode_selected
            and not busy
        )
        problems: list[str] = []
        if panel.offline_enabled.isChecked() != bool(offline.enabled):
            problems.append("番劇儲存勾選狀態與核心不同步")
        if panel.offline_enabled.isEnabled() != bool(
            parent.enabled and offline.available and not busy
        ):
            problems.append("番劇儲存開關可用狀態錯誤")
        if panel.offline_save_button.isEnabled() != expected_ready:
            problems.append("保存按鈕沒有依作品／集數選取狀態切換")
        if panel.offline_import_button.isEnabled() != expected_ready:
            problems.append("本機媒體匯入按鈕狀態錯誤")
        if panel.offline_verify_button.isEnabled() != expected_ready:
            problems.append("保存完整性驗證按鈕狀態錯誤")
        expected_cancel = bool(
            busy
            and panel.operation in {"offline-import", "offline-verify"}
            and not panel.offline_cancel_event.is_set()
        )
    ...
    return _item(
        "ui.ani_gamer_offline_actions",
        "block" if problems else "pass",
        "動畫瘋番劇儲存按鈕異常" if problems else "動畫瘋番劇儲存按鈕正常",
        "; ".join(problems)
        if problems
        else "已核對父／子 MOD、作品／集數選取、保存、匯入、驗證與取消狀態。",
        "ui.ani_gamer_offline.sync" if problems else "",
    )


def _ani_gamer_episode_actions_item(
    context: object, owner: object | None
) -> SelfCheckItem:
    """Verify the current series, episode and 403 fallback control states."""
    ...
    if parent is None or search is None or episodes is None:
        return _item(
            ...
            "block",
            "動畫瘋作品／集數 MOD 未載入",
            "父 MOD、搜尋子 MOD 或集數子 MOD 缺少執行狀態。",
            "ui.ani_gamer_episode.restore",
        )
    if owner is None:
        return _item(
            "ui.ani_gamer_episode_actions",
            "warning",
            "未附加動畫瘋作品／集數工作區",
            "核心狀態可用；從主視窗執行可再核對載入與 403 備援按鈕。",
            "ui.ani_gamer_episode.open",
        )
    panel = owner.optional_workspace_manager.panels.get("ani-gamer")
    if panel is None:
        valid = not parent.enabled
        return _item(
            ...
            "父 MOD 停用時不建立工作區。"
            if valid
            else "父 MOD 已啟用，但工作區尚未同步建立。",
            "ui.optional_workspaces.sync" if not valid else "",
        )

    try:
        selected = panel.selected_result()
        selected_episode = panel.selected_episode()
        busy = bool(panel.busy)
        problems: list[str] = []
        if panel.search_button.isEnabled() != bool(
            parent.enabled and search.enabled and not busy
        ):
            problems.append("搜尋按鈕狀態錯誤")
        if panel.load_episodes_button.isEnabled() != bool(
            parent.enabled
            and episodes.available
            and selected is not None
            and not busy
        ):
            problems.append("顯示選取作品集數按鈕狀態錯誤")
        manual_ready = bool(
            parent.enabled and episodes.enabled and selected is not None and not busy
        )
        if panel.manual_episode_url.isEnabled() != manual_ready:
            problems.append("官方單集網址欄位狀態錯誤")
        if panel.manual_episode_add.isEnabled() != bool(
            manual_ready and panel.manual_episode_url.text().strip()
        ):
            problems.append("加入官方單集網址按鈕狀態錯誤")
        if panel.open_episode_button.isEnabled() != bool(
    ...
    return _item(
        "ui.ani_gamer_episode_actions",
        "block" if problems else "pass",
        "動畫瘋作品／集數按鈕異常" if problems else "動畫瘋作品／集數按鈕正常",
        "; ".join(problems)
        if problems
        else "已核對搜尋、作品選取、集數載入、官方單集網址與 403 備援狀態。",
        "ui.ani_gamer_episode.sync" if problems else "",
    )
...
def collect_ui_self_check_items(
    ...
    return (
        ...
        _ani_gamer_episode_actions_item(context, owner),
        _ani_gamer_offline_actions_item(context, owner),
    )
