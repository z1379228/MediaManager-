...
Development 39.0.7 provides the tested split-phase Stable operator and compiles
the production Ed25519 public identity. The private key remains outside the
repository. The old single-command Stable procedure remains prohibited. After
39.0.7 stage／commit／source freeze and separate build authorization, create only
the receipt-bound handoff:
