...
3. Run `python -m tools.build_version`. A key change therefore requires a new build.
4. Run `python -m tools.sign_release --root Version/1.0 --private-key <external-key-path>`.
5. Run `python -m tools.release_preflight --root Version/1.0`. It must print `READY`.
6. Distribute the EXE, `mod/builtin` files, and `security` manifest/signature
   together.
