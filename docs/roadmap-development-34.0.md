...
1. 同一時間只允許一個 `READY` 項目進入 `IN_PROGRESS`；freeze 前 106 路徑快照已由
   ...
   下一項是 35.0 的 G35-02，目前為 `READY`。34.0 不建立 EXE，
   已驗證的 runtime 不需重新下載。
...
### FIX-34-07｜P1｜TRANSFERRED TO G37／G38 / BLOCKED｜外部網站與真實 UI 驗證

- **Blocker**：動畫瘋單一官方單集 live smoke 已完成，其他 provider 官方網站、940×620 真實視窗、鍵盤焦點與讀屏仍需受控環境人工驗證；不得繞過登入、Cookie、Cloudflare、DRM、廣告或付費限制。
...
- **Minimum-cost next validation**：若再次出現，先記錄精確時間與當時操作，按「確定」讓程序結束後立即保存對應 Event 1000/1001、faulting module／offset 與新 dump；不要按「取消」進入未配置的除錯器。取得模組證據後才建立對應 regression／修正。
...
- **Transient observation**：較早一次完整 run 曾有 3 個非動畫瘋 provider 子程序無結果退出（其餘 `1002 passed, 5 skipped`），沒有 stderr、WER 1000／1001 或原生對話框。三個精確案例立即重跑為 `3 passed`；本輪確認 root cause 是 child 已退出後 stdout reader 超過固定 1 秒才入 queue 的競態，已改用既有 operation／idle deadline 並加入 1.25 秒受控回歸，最新完整 suite 為 `1092 passed, 7 skipped`。
...
| ANI-34-02 | P0 | `MONITORING / NOT_REPRODUCED` | 保存 0x80000003 截圖證據；受控 UI、WebEngine lifecycle 與完整 suite 未重現。若再次發生，先取得同時段 WER faulting module／offset 與 dump，再做最小 regression／修正。 |
...
| VUP-34-05 | P1 | `TRANSFERRED / EXTERNAL BLOCKED` | 合法 provider crash 證據由 G37-01 管理；940×620、鍵盤、四語與讀屏人工驗證由 G38-01 管理。CLI exit 0 不得代替人工證據。 |
