...
1. 同一時間只允許一個 `READY` 項目進入 `IN_PROGRESS`；freeze 前 106 路徑快照已由
   ...
   35.0～37.0 source-only Exit Gate 已依序完成；G38-01 的無頭自動基線、兩個 P1 修正、source
   freeze 與 Development 38.0 包已固定為不可覆寫基線，人工矩陣仍為 `MANUAL BLOCKED`。
   G38-02 已於 38.1.15 依使用者範圍決定關閉並移除，狀態為
   ...
   `SOURCE VALIDATED / ANI-GAMER MOD REMOVED / NO PACKAGE / SAFE_MODE`。G39-01 已完成為
   `MEASURED / NO CHANGE / NO RELEASE / SAFE_MODE`，目前沒有 `ACTIVE / IN_PROGRESS` 工作；
   G40-01 保持 `WAITING / AUTHORIZATION + STAGED VERIFY/HEADLESS SAFE_MODE EVIDENCE REQUIRED`，
   G38-01 的人工矩陣仍為
   `MANUAL BLOCKED / SAFE_MODE`。以下 38.1.0～38.1.14 細節只作歷史追溯。
   2026-07-22 截圖已使作品、選取 context、兩筆集數列、選取單集、單集操作、offline metadata
   index 與 codec fail-closed presentation 的可見矩陣通過；離線區仍明示未連結影片或字幕。
   OS handoff、系統瀏覽器播放、38.1.8 failure-state preservation 與 38.1.9
   切換作品後拒絕舊集數回應仍待人工
   證據；38.1.10 只收斂 exact-host canonical `sn=<ASCII digits>` query，38.1.11 再要求 canonical
   item ID／URL identity、history／offline 讀寫驗證並在 busy／closing 時拒絕舊單集 handoff。38.1.11 截圖
   不含 exact URL，因此無法證明 OS handoff 對應選取單集；38.1.12 將四語 `episode_opened`
   加入 `{url}`，並由選取單集／history handoff 傳入 exact URL，同時明示頁面載入／播放未確認。
   38.1.13 再修正相容性彈窗固定誤用 `catalog_opened`，以及 OS 拒絕或 opener `RuntimeError` 只更新
   被 modal 遮住主狀態的缺口：依 exact route 選用 `series_opened`／`episode_opened`，單集保留
   exact URL，並把成功／失敗文字鏡像到 dialog-local status。不改 provider、network、Cookie、
   download 或 stream。
   所有修正都不構成播放完成證據。38.1.11 初始 RED／GREEN 為 `4 failed in 1.80s`／`6 passed in 1.26s`，
   read-side RED／GREEN 為 `2 failed in 0.52s`／`3 passed in 0.98s`，最終相關非 GUI 組
   `129 passed, 1 skipped in 7.23s` 與來源 Gate 通過。38.1.12 RED／GREEN 為
   `1 failed in 0.42s`／`2 passed in 0.94s`，精準相關來源組 `9 passed in 1.04s`，擴大非 UI 組
   `143 passed, 2 skipped in 1.60s`；Ruff／文字污染 `368 / 573`、MOD `8 / 4`、網站
   `13 / 42 / 56`、依賴鎖 `10`、版本文件 `4`、保留版本 `5`、版本文件測試
   `6 passed in 0.33s`、Repository 外 compileall、CLI 38.1.12、Version `.pyc` 零污染與 diff check 通過。
   38.1.13 精準 RED／GREEN 為 `1 failed in 1.27s`／`1 passed in 0.71s`，擴大非 GUI 組為
   `115 passed, 1 skipped in 3.77s`，版本／文件組為 `19 passed in 1.21s`，其餘來源 Gate 已通過；人工
    38.1.13 截圖仍待，故維持 `SOURCE VALIDATED / LIVE WAITING / SAFE_MODE`。沒有
    stage、commit、freeze、build、package 或 push。
   38.1.14 修正 `dispatch_official_url` 的 transport positional `url` 與 `episode_opened` message
   placeholder `url=` 在 Python 綁定時衝突、於 helper body 前拋出 multiple-values `TypeError` 的
   缺口；只把前者改名為 `official_url` 並同步 opener／失敗文案引用，保留後者。不改 provider、
   network、Cookie、download 或 stream。runtime 擴大組修正前為
   ...
   同步後重跑為 `135 passed, 1 skipped in 1.87s`，其餘來源 Gate 亦通過，故該輪維持
   `SOURCE VALIDATED / LIVE WAITING / SAFE_MODE`。人工
   checklist 使用 38.1.14；沒有 stage、commit、freeze、build、package 或 push。
   新增的 38.1.14 單張 evidence-only 截圖
   `codex-clipboard-5687e504-c306-4d3a-86b0-cc79cf9d8567.png`（SHA-256
   `5C52283BACEB1EECB5E55F92A1301D1E6240239D0F2BD6CDA1C1BEEB05DE5E42`）只證明「盜墓王 [1]」
   單集情境、緊湊 codec fallback 與 system-browser 建議操作可見；判定只包含
   `embedded_fail_closed_presentation=PASS`、`episode_context_visible=PASS`、
   `system_browser_action_visible=PASS` 與 `embedded_playback=EXPECTED_UNSUPPORTED`。CLI 另核對
   當時來源 38.1.14。畫面沒有證明按鈕已點、OS handoff、
   官方頁載入、廣告或播放，故 ownership 與 `LIVE WAITING / SAFE_MODE` 不變，純證據同步不升版。
   BoringMan314/aniGamerPlus V24.9.10（`ce7a6d5`）的唯讀差異複核亦未找到 G38 scope 內、
   非重複且有 failing regression 的安全 material delta；通用功能已吸收，GPL 程式碼未搬入 MIT
   專案，串流／Cookie／Cloudflare／廣告規避仍排除。此掃描記為 `ABSORBED / NO RELEASE`，
   該輪曾判定不建立下一修正號；此判定後續由使用者的移除決策取代，38.1.15 只記錄
   動畫瘋整合移除。本機 MP4 `faststart` 只登記為 G39 evidence-gated 候選，沒有可重現缺口時
   不執行。相關非 UI／
   版本／文件組以唯一 LOCALAPPDATA test-root 重跑為 `173 passed, 1 skipped in 5.36s`，其餘
   quality、MOD、網站、依賴、版本、compileall 與 diff Gate 均通過。
   官方服務條款沒有第三方可保存來源且禁止重製、廣告規避與串流破解，故影片下載維持
   `BLOCKED / NO AUTHORIZED MEDIA SOURCE`。G39-01 measurement baseline 已完成且沒有可證瓶頸；
   G40-01 未取得逐項明確授權前不得啟動。
   34.0 不建立 EXE，
   已驗證的 runtime 不需重新下載。
...
- **Blocker**：G37 去識別 provider／原生證據邊界已完成；動畫瘋單一官方單集 live smoke 已完成，
  940×620 真實視窗、鍵盤焦點、讀屏、高對比與四語仍需受控人工環境。G38-02 只允許
  AniGamer exact-host、session-only Cookie，且不包含 Cloudflare 驗證狀態；不得擷取瀏覽器
  Cookie、攔截播放串流，或繞過登入、Cloudflare、DRM、廣告、付費與地區限制。
