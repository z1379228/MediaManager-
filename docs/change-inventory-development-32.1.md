...
這些授權不包含 push、build、EXE、Testing／Stable、簽署、發布或上傳；G40-01 現為
build waiting，仍缺 staged candidate／headless 證據。
...
| 40.0 | `BUILD WAITING / STAGED CANDIDATE + HEADLESS EVIDENCE REQUIRED` | 2026-07-22 source-only `python -B main.py --verify-only` 已 exit 0 並精確輸出 `MediaManager security mode: SAFE_MODE`。39.0.6 已固定為目前 source freeze；build／candidate、EXE、Testing／Stable、簽署、發布、上傳與 push 皆未授權。 |
