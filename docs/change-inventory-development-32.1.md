...
G38 AniGamer 證據邊界：使用者提供的 Development 37.0 截圖是 pre-fix baseline，記錄內嵌
H.264／AAC／HLS／MSE 不可用與集數 403／瀏覽器驗證安全降級；現行修正只提升系統瀏覽器
主操作的可見性，不改 network policy，也未實作 Cookie／串流攔截或 DRM、廣告、登入、地區
限制繞過。UI 採截圖優先、禁止自動互動；截圖不能完成 Tab、UIA／NVDA／讀屏或 Windows OS
高對比 Gate，因此 G38 仍保持 `ACTIVE / MANUAL REVALIDATION BLOCKED / SAFE_MODE`。
...
2. 後續唯一版本順序與優先度由 `docs/roadmap-development-34.0-40.0.md` 管理；
   35.0～37.0 Exit Gate 已完成且維持 `SOURCE ONLY / SAFE_MODE`；G38-01 現為唯一
   `ACTIVE / P1 FIXES VALIDATED / MANUAL REVALIDATION BLOCKED` 項目，39.0～40.0 保持 `WAITING`，沒有平行
   `IN_PROGRESS` 工作。
   既有 41.0～43.0 只保留歷史，
