...
- Ruff 全專案檢查通過；Pytest 632 項通過，另有 2 項 Windows symlink 測試因帳戶權限
  跳過。
...
3. 正式候選 evidence 尚須綁定最終來源 revision、成品 digest、產生時間與發布工具版本，
   並完成升級、回退、MOD 接線與完整 copied-folder 驗證。
4. `generic-ytdlp` 仍是預設停用的舊 Beta 多網域相容 provider；它不影響 YouTube／
