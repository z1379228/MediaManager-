...
| 6 | G38-01 | 38.0-P1 | `ACTIVE / P1 FIXES VALIDATED / MANUAL REVALIDATION BLOCKED / SAFE_MODE` | 焦點指示、安全 badge 與 AniGamer 系統瀏覽器主操作可見性已完成低風險修正；AniGamer targeted suite 60 passed／1 skipped，完整 Repository 1281 passed／7 skipped；940×620 全工作區、NVDA／UIA、OS 高對比與四語人工矩陣仍需人工環境。 |
...
| 8 | G40-01 | 40.0-P0 | `WAITING / PRIOR GATES + FORMAL RUNTIME REQUIRED` | 新候選 copied-folder 的精確 `SAFE_MODE` 證據與可回退性；本次單一 38.0 打包授權不啟動 G40。 |
...
來源身分已同步為 `38.0.0`；目前 35.0～38.0 核准變更的 stage、本機 commit、Development 38.0
source freeze，以及一次 SAFE_MODE 未簽署候選包的 build／EXE 均已獲明確授權。本次
source-freeze commit 提供打包所需 revision；產物結果只以後續實際 build 稽核為準。
...
- **Verified gap／Unknown**：copied-folder／rollback 本體列為 `ABSORBED`；已確認尚缺精確
  SAFE_MODE stdout 證據與 Stable CLI wiring。一次 Development 38.0 build／EXE 授權已取得；
  正式 runtime 驗證與正式簽署身分仍未知。
