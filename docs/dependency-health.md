...
Windows 正式資料夾版預設攜帶固定版本、通過 SHA-256 驗證的 Deno，位置為
`Version/<version>/tools/deno.exe`。Deno 的快取與下載資料仍由 Deno 自身放在使用者資料位置，不會寫回唯讀的版本目錄。

1.3.3 起不再只檢查套件名稱是否存在：yt-dlp 需為 2026.7.4 以上、
