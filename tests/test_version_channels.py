...
def test_development_display_and_release_tracks_are_explicit() -> None:
    assert display_version() == "開發版 39.0.12"
    ...
    assert release_identity_version("development") == "39.0.12"
    assert release_identity_version("testing") == "1.2.1"
...
def test_testing_display_preserves_the_correction_number(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(version_module, "BUILD_CHANNEL", "testing")

    assert version_module.display_version() == "測試版 1.2.1"


def test_development_39_rejects_manifest_upper_bound_32_1() -> None:
    assert is_core_compatible("0.1.0", "39.0.12")
