...
- `Version/Development/<major>.<minor>/` 與 `Version/Stable/<major>.<minor>/`：
  雙軌工具遷移後的新路徑；正式版必須經使用者確認才建立。
...
- 開發版與正式版使用獨立編號。開發版沿用 `5.0`、`6.0` 世代；正式版從
  `1.0`、`1.1` 依穩定功能遞增，不可用數字大小跨通道比較新舊。
